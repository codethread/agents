...
function openExternalEditor(
	...
	if (result.signal) return { ok: false, message: `editor terminated by signal ${result.signal}` };
...
function toMarkdownFileNameStem(value: string): string {
	const normalized = value
		...
		.replace(/[^a-z0-9]+/g, "-")
		.replace(/^-+|-+$/g, "");
	return normalized || "last-message";
}

export async function editMarkdownInExternalEditor(
	markdownBody: string,
	options: { fileNameStem?: string } = {},
): Promise<EditedFileResult> {
	...
	const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-last-message-"));
	const filePath = path.join(
		tmpDir,
		`${toMarkdownFileNameStem(options.fileNameStem ?? "last-message")}.md`,
	);

	try {
		await fs.writeFile(filePath, markdownBody, "utf-8");
		const result = openExternalEditor(editor, filePath);
		if (!result.ok) return { ok: false, level: "error", message: result.message };
