...
- For `nvim` (including when it is set in `$VISUAL` or `$EDITOR`), it restores the terminal before starting the editor and resumes Pi after it closes. This lets `nvim` use the TTY; it cannot be non-blocking in the same terminal. The default local editor is therefore also opened in the terminal.
...
- `--debug-nonblocking-editor` prints the configured editor command, its source, and whether it uses the terminal, then exits.
