...
A Pi subagent is a separate native session and resolves its own Millstrand identity from its actual child session ID. Child spawn removes inherited `MILLSTRAND_AGENT_ID`, `MILLSTRAND_RUN_ID`, `MILLSTRAND_WORKSPACE`, `MILLSTRAND_MANAGED_BOOTSTRAP`, `MILLSTRAND_MANAGED_GUIDANCE`, `MILLSTRAND_BOOTSTRAP_*`, reservation, and identity-transport state. It preserves resources, `--agent`, model selection, and native resume arguments.

When a parent identity is known, the spawn passes it only as private `MILLSTRAND_PI_PARENT_IDENTITY` attribution; the Harnesses-owned identity data lifecycle translates that to `strand identity startup --parent-identity`. The parent's cwd-resolved absolute workspace is passed separately as `MILLSTRAND_PI_WORKSPACE`, so delegation to another cwd cannot reinterpret a relative route. The parent context reaches this extension over Pi's shared `pi.events` bus and is checked against the active native session ID before spawning; it does not rely on cross-extension module caching. Neither value grants the child parent ownership, and both are replaced for a grandchild after the child resolves its own identity.
...
For example, `deepseek/deepseek-flash:max` selects Flash with native `max` thinking;
...
Both manifest writers canonicalize the session directory before queueing updates. Concurrent writes within the parent process stay serialized even when the directory is reached through a symlink and the manifest is being created for the first time.
