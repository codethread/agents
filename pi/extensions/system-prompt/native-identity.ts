...
export type NativeIdentityState =
	| { status: "pending" }
	| ({ status: "bound" } & NativeIdentityResult)
...
function parseStartupResponse(stdout: string): StartupResponse {
	...
	if (response.operation !== "identity startup") {
		throw new Error("Strand returned an unexpected identity startup operation.");
	}
...
function diagnostic(result: { stdout: string; stderr: string }): string {
	return (result.stderr.trim() || result.stdout.trim() || "no diagnostic output")
		.replace(/\s+/g, " ")
		.slice(0, 500);
}

export function isLegacyManagedPiEnvironment(env: NodeJS.ProcessEnv = process.env): boolean {
...
export function getNativeIdentityInputs(
	...
	return {
		identity: optionalString(pi.getFlag(MILLSTRAND_IDENTITY_FLAG), `--${MILLSTRAND_IDENTITY_FLAG}`),
		parentIdentity: optionalString(
			env[MILLSTRAND_PARENT_IDENTITY_ENV],
			MILLSTRAND_PARENT_IDENTITY_ENV,
		),
		workspace:
			optionalString(pi.getFlag(MILLSTRAND_WORKSPACE_FLAG), `--${MILLSTRAND_WORKSPACE_FLAG}`) ??
...
export async function resolveNativeIdentity(
	...
	if (options.thinkingLevel) args.push("--thinking-level", options.thinkingLevel);
	...
	if (result.code !== 0) {
		throw new Error(`Strand identity startup failed (exit ${result.code}): ${diagnostic(result)}`);
	}
...
export function formatNativeIdentityState(state: NativeIdentityState, cwd: string): string {
	const details =
		state.status === "bound"
			? {
...
export function nativeIdentityModel(ctx: Pick<ExtensionContext, "model">): string | undefined {
	return ctx.model?.id;
}
