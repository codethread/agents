...
describe("resolveNativeIdentity", () => {
	...
	it("surfaces unavailable Strand and malformed responses without fabricating identity", async () => {
		...
		).rejects.toThrow("Strand identity startup failed (exit 1): connection refused");
		...
		const malformed = vi.fn(async () => ({
			stdout: '{"operation":"identity startup","identity":"invented-without-contract"}',
			stderr: "",
			code: 0,
			killed: false,
		}));
...
describe("native identity inputs", () => {
	...
	it("suppresses only legacy managed runs", () => {
		expect(isLegacyManagedPiEnvironment({ MILLSTRAND_RUN_ID: "run-1" })).toBe(true);
		expect(isLegacyManagedPiEnvironment({ MILLSTRAND_AGENT_ID: "ambient-parent" })).toBe(false);
