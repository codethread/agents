...
function fixture(
	...
	const context = {
		...
		"appended-system-prompts": overrides.appends ?? ["first frozen", "same", "same"],
	};
	...
	if (selection.kind !== "native-v1") throw new Error("fixture did not select native-v1");
	return { runId, attempt, invocation, session, metadata, bootstrap, bundle, env, selection };
}
...
describe("managed Pi guidance selection", () => {
	it("preserves unmanaged identity and old-spool legacy suppression when guidance is absent", () => {
		expect(selectManagedPiGuidance("session", cwd, {})).toEqual({ kind: "unmanaged" });
		expect(
			selectManagedPiGuidance("session", cwd, { MILLSTRAND_AGENT_ID: "ambient-parent" }),
		).toEqual({ kind: "unmanaged" });
		expect(selectManagedPiGuidance("session", cwd, { MILLSTRAND_RUN_ID: "old-run" })).toEqual({
			kind: "legacy",
	...
	it("rejects missing, changed, oversized, provider, session, and fence metadata", () => {
		...
		const cases: Array<[string, NodeJS.ProcessEnv]> = [
			["missing bootstrap", { MILLSTRAND_MANAGED_GUIDANCE: JSON.stringify(f.metadata) }],
			[
				"provider",
				{
					...f.env,
					MILLSTRAND_MANAGED_GUIDANCE: JSON.stringify({ ...f.metadata, harness: "codex" }),
				},
			],
			[
				"session",
			...
			[
				...
				{
					...f.env,
					MILLSTRAND_MANAGED_BOOTSTRAP: JSON.stringify({ ...f.bootstrap, attempt: 2 }),
				},
			],
			["oversized", { MILLSTRAND_MANAGED_GUIDANCE: `{"x":"${"x".repeat(70_000)}"}` }],
		];
	...
	it("never lets a Pi child consume inherited root guidance", () => {
		const f = fixture();
		expect(selectManagedPiGuidance(f.session, cwd, { ...f.env, PI_SUBAGENT: "1" })).toEqual({
			kind: "unmanaged",
		});
	});
});
...
describe("managed Pi bundle and receipts", () => {
	it("requests every launch fence and preserves fresh/resume/retry identity", async () => {
		for (const f of [
			...
			await expect(fetchManagedGuidance(f.selection, f.session, command, f.env)).resolves.toEqual(
				f.bundle,
			);
	...
	it("rejects malformed, duplicate-key, oversized, digest, session, capability and invocation responses", async () => {
		...
		for (const stdout of changed) {
			const command: BoundedCommand = async () => ({ stdout, stderr: "", code: 0 });
			await expect(fetchManagedGuidance(f.selection, f.session, command, f.env)).rejects.toThrow();
		}
	});
	...
	it("renders identity first, preserves duplicate positions and appends one current-run footer", () => {
		...
		expect(contribution.indexOf(instruction)).toBeLessThan(contribution.indexOf("first\n"));
	...
	it("rejects rendered contributions above the Pi profile bound without truncation", () => {
		...
		expect(() => renderManagedGuidance(f.bundle)).toThrow("exceeds 65536 bytes");
	});
	...
	it("accepts acknowledgement replay, rejects ignored receipts, and records fenced failure", async () => {
		...
		const receipt = JSON.parse((failure.mock.calls[0]?.[0] as string[]).at(-1) ?? "null");
