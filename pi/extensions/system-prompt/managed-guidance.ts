...
import { parseStrictJson, sha256CanonicalJson } from "../shared/strict-json.js";
...
function parseMetadata(raw: string): { transport: "legacy" } | ManagedGuidanceMetadata {
	const value = object(parseStrictJson(raw, METADATA_MAX_BYTES), MANAGED_GUIDANCE_ENV);
	...
	if (value.harness !== "pi") throw new Error("managed guidance provider is not pi.");
	return {
		...
		"capability-sha256": digest(value["capability-sha256"], "capability-sha256"),
	};
}
...
function parseBundle(
	...
	const value = object(parseStrictJson(stdout, BUNDLE_MAX_BYTES), "guidance bundle");
	closedKeys(value, [
		"schema",
		"operation",
		"run-id",
	...
	closedKeys(contextValue, ["schema", "identity-instruction", "appended-system-prompts"]);
	...
	const appends = contextValue["appended-system-prompts"].map((entry, index) => {
		if (typeof entry !== "string" || !entry.trim()) {
			throw new Error(`managed context append ${index} must be a nonblank string.`);
		}
		return entry;
	});
	const bundle: ManagedGuidanceBundle = {
		...
		harness: "pi",
		"native-session-id": string(value["native-session-id"], "native-session-id"),
		...
		context: {
			schema: "millstrand.agent-managed-context/v1",
			"identity-instruction": string(contextValue["identity-instruction"], "identity-instruction"),
	...
	for (const key of [
		...
		if (bundle[key] !== expected[key]) throw new Error(`guidance bundle ${key} fence mismatch.`);
	}
	...
	const calculated = sha256CanonicalJson([bundle["run-id"], bundle.workspace, bundle.context]);
	if (calculated !== bundle["bundle-sha256"]) throw new Error("guidance bundle digest mismatch.");
	return bundle;
}
...
export const runBoundedStrand: BoundedCommand = (args, options) =>
	new Promise((resolvePromise, reject) => {
		...
		const append = (current: Buffer, chunk: Buffer, maximum: number, label: string) => {
...
function strandBase(selection: Extract<ManagedPiSelection, { kind: "native-v1" }>): string[] {
	return [
		"--workspace",
...
function validateReceiptResult(
	...
	const value = object(parseStrictJson(stdout, METADATA_MAX_BYTES), "guidance receipt result");
	closedKeys(value, [
		"schema",
		"result",
		"state",
	...
	if (value.schema !== RECEIPT_RESULT_SCHEMA) throw new Error("receipt result schema is invalid.");
	if (!["recorded", "replayed", "ignored"].includes(String(value.result))) {
		throw new Error("receipt result is invalid.");
	}
	if (!["pending", "fetched", "acknowledged", "failed"].includes(String(value.state))) {
		throw new Error("receipt state is invalid.");
	}
	const expected = receiptBase(selection, nativeSessionId);
	for (const [key, expectedValue] of Object.entries(expected)) {
		if (key === "schema") continue;
		if (value[key] !== expectedValue) throw new Error(`receipt result ${key} fence mismatch.`);
	}
	return value;
}
...
export async function acknowledgeManagedGuidance(
	...
	const receipt = { ...receiptBase(selection, nativeSessionId), outcome: "adapter-handoff" };
	...
	const response = validateReceiptResult(result.stdout, selection, nativeSessionId);
	if (response.result === "ignored" || response.state !== "acknowledged") {
		throw new Error("Strand did not accept the current adapter-handoff receipt.");
	}
}
...
export async function failManagedGuidance(
	...
	const result = await command(
		[...strandBase(selection), "agent", "guidance", "fail", "--receipt", JSON.stringify(receipt)],
		{ cwd: selection.bootstrap.cwd, env, signal },
	);
	commandFailure(result, "Strand managed guidance failure recording");
	const response = validateReceiptResult(result.stdout, selection, nativeSessionId);
	if (response.result === "ignored" || response.state !== "failed") {
		throw new Error("Strand did not record the current managed guidance failure.");
	}
}
	...
	return JSON.stringify(
		{
			...
			...(selection.kind === "native-v1"
				...
				: {}),
			fetched: bundle !== null,
			...(bundle ? { identity: bundle.identity, workspace: bundle.workspace } : {}),
			...(error ? { error } : {}),
		},
		null,
		2,
