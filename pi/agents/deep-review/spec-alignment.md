...
You have readonly tools only. Before reviewing, read `plugins/coding/skills/robustness/SKILL.md` if it exists and use it when evaluating specified edge cases, failure modes, fallbacks, validation, retries, and error handling. This role is appropriate only when the review brief provides a spec, task file, issue text, plan, acceptance criteria, or an explicit path to a reference document. If no such reference is provided, return exactly:
