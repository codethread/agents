...
function assertUnicode(value) {
	for (let index = 0; index < value.length; index += 1) {
		...
		if (code >= 0xd800 && code <= 0xdbff) {
			...
			index += 1;
		} else if (code >= 0xdc00 && code <= 0xdfff) invalid("unpaired low surrogate");
	}
}

export function parseStrictJson(text, maxBytes) {
	if (Buffer.byteLength(text, "utf8") > maxBytes) invalid(`input exceeds ${maxBytes} bytes`);
	...
	const value = () => {
		...
		if (character === "{") {
			...
			for (;;) {
				...
				keys.add(key);
				whitespace();
				if (text[position++] !== ":") invalid(`expected ':' at byte ${position - 1}`);
				result[key] = value();
				whitespace();
				if (text[position] === "}") {
					position += 1;
...
export function hashDirectory(
	...
	const visit = (directory) => {
		for (const name of readdirSync(directory).sort()) {
			...
			if (stats.isSymbolicLink()) throw new Error(`closure contains a symbolic link: ${path}`);
...
export function parseGuidance(raw, harness = "codex") {
	const value = object(parseStrictJson(raw, METADATA_MAX_BYTES), "managed guidance");
	if (value.transport === "legacy") {
		closed(value, ["schema", "transport"]);
		if (value.schema !== GUIDANCE_SCHEMA) throw new Error("unsupported managed guidance schema");
	...
	if (
		...
		throw new Error("unsupported managed guidance schema, transport, or provider");
	}
	return {
...
async function runStrand(args, cwd) {
	const executable = process.env.MILLSTRAND_CODEX_STRAND_BIN?.trim() || "strand";
	return await new Promise((resolvePromise, reject) => {
		...
		const timer = setTimeout(() => fail(new Error("Strand request exceeded 3 seconds")), 3000);
	});
}
...
function parseBundle(stdout, guidance, bootstrap, nativeSessionId) {
	...
	if (
		...
		throw new Error("guidance bundle schema, operation, provider, or transport is invalid");
	const context = object(value.context, "managed context");
	closed(context, ["schema", "identity-instruction", "appended-system-prompts"]);
	if (
	...
	for (const [index, append] of context["appended-system-prompts"].entries())
		if (typeof append !== "string" || !append.trim())
			throw new Error(`managed context append ${index} is blank or not a string`);
	const bundle = {
		...value,
		...
		"native-session-id": nonblank(value["native-session-id"], "native-session-id"),
	...
	if (
		sha256CanonicalJson([bundle["run-id"], bundle.workspace, context]) !== bundle["bundle-sha256"]
	)
...
function parseReceipt(stdout, guidance, nativeSessionId, state) {
	const value = object(parseStrictJson(stdout, METADATA_MAX_BYTES), "receipt result");
	closed(value, [
		"schema",
		"result",
		"state",
	...
	for (const [key, expected] of Object.entries(receiptBase(guidance, nativeSessionId)))
		if (key !== "schema" && value[key] !== expected)
			throw new Error(`receipt result ${key} fence mismatch`);
	if (value.result === "ignored") throw new Error("current receipt was ignored");
}

async function sendReceipt(kind, guidance, bootstrap, nativeSessionId, details = {}) {
...
async function handoff(nativeSessionId, cwd, eventName) {
	...
	try {
		...
		stage = "validation";
		const bundle = parseBundle(startup.stdout, guidance, bootstrap, nativeSessionId);
		stage = "rendering";
