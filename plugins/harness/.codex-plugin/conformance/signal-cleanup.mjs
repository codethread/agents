...
async function waitForProcessGroupExit(processGroupId) {
	...
	assert.equal(processGroupExists(processGroupId), false, `process group ${processGroupId} leaked`);
}
...
function waitForProbe(child, stderr) {
	return new Promise((resolvePromise, reject) => {
		...
		const timer = setTimeout(() => {
			reject(new Error(`interrupt probe did not become ready; stderr: ${stderr.value}`));
		}, 5_000);
...
async function checkSignal(signal) {
	const root = mkdtempSync(join(tmpdir(), `codex-hook-${signal.toLowerCase()}-`));
	const stderr = { value: "" };
	...
	try {
		...
		assert.equal(processGroupExists(processGroupId), true, "probe child group must be running");
		...
		assert.deepEqual(readdirSync(root), [], `${signal} must remove disposable directories`);
	} finally {
...
if (process.platform === "win32") {
	console.log("Codex hook interruption cleanup skipped: process groups require POSIX.");
} else {
	await checkSignal("SIGINT");
	await checkSignal("SIGTERM");
	console.log("Codex hook interruption cleanup passed (SIGINT, SIGTERM; disposable CLI-only).");
}
