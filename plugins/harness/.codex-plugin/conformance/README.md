...
No desktop app, plugin cache, or live conversation is opened, restarted, reloaded, or modified by this suite. Desktop execution is intended by the documented package contract but is not runtime-certified and is not a gate. No-model discovery proves registration metadata, not host execution or model delivery. A separate disposable CLI model smoke is needed to prove first-request developer-context visibility for a particular host build.
