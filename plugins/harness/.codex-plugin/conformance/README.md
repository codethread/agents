...
A child uses `SubagentStart`. In 0.154.0, `additionalContext` is extra **developer context**, not a replacement system prompt. The packaged handlers set `additionalContextLimit` to 4,096 tokens, while `identity.sh` rejects context above 3,072 bytes, so required identity guidance is never intentionally sent through Codex's spill-preview path. Failures return bounded `systemMessage` warnings without an identity instruction.
