...
import {
	...
	parseStrictJson,
	sha256CanonicalJson,
} from "../plugins/harness/.codex-plugin/lib/managed-guidance.mjs";

const scriptPath = fileURLToPath(import.meta.url);
const packageRoot = resolve(dirname(scriptPath), "..");
const codexPluginRoot = join(packageRoot, "plugins/harness");
const piOwner = join(packageRoot, "pi/extensions/system-prompt/index.ts");
...
function fail(code, diagnostic) {
	if (!failureCodes.has(code)) throw new Error(`internal unsupported failure code: ${code}`);
	return {
...
function parseRequest(raw) {
	const value = object(parseStrictJson(raw, METADATA_MAX_BYTES), "preflight request");
	const required = [
		"schema",
		"harness",
		"executable",
	...
	closed(value, [...required, "model", "effort", "native-session-id"], required);
	if (value.schema !== REQUEST_SCHEMA) throw new Error("unsupported preflight request schema");
	if (!new Set(["codex", "pi"]).has(value.harness)) throw new Error("unsupported harness");
	...
	if (typeof value.resumes !== "boolean") throw new Error("resumes must be boolean");
...
function run(command, args, request, { input = "", timeout = 5_000 } = {}) {
	return new Promise((resolvePromise, reject) => {
		...
		const timer = setTimeout(() => rejectOnce(new Error(`probe exceeded ${timeout}ms`)), timeout);
...
function parseSelectors(argv, harness) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (
			...
			if (typeof attached !== "string") throw new Error("Codex config selector has no value");
			if (
		...
		if (harness === "codex" && argument === "--dangerously-bypass-hook-trust") {
			throw new Error("Codex hook trust bypass cannot establish native-v1 capability");
		}
...
function codexConfigEvidence(request) {
	...
	for (const path of [...new Set(paths)]) {
		...
		if (/^[\t ]*["']?[^\n=]*instructions[^\n=]*["']?[\t ]*=/im.test(source)) {
			throw new Error(`competing Codex instruction configuration is present in ${path}`);
		}
...
async function preflightCodex(request) {
	...
	let listed;
	try {
		listed = object(await listCodexHooks(request, parsed.selectors), "hooks/list result");
	} catch (error) {
		return fail("probe-failed", error.message);
	}
	if (!Array.isArray(listed.data) || listed.data.length !== 1)
		return fail("unverifiable-profile", "hooks/list did not return exactly one cwd profile");
...
function expandHome(value, env) {
	...
	if (value.startsWith("~/")) return env.HOME ? join(env.HOME, value.slice(2)) : undefined;
	return value;
}
...
function packageExtensions(settings, settingsPath, env) {
	...
	for (const entry of settings.packages) {
		...
		const manifest = object(
			parseStrictJson(readFileSync(join(root, "package.json"), "utf8"), METADATA_MAX_BYTES),
			"package manifest",
		);
		const declared =
	...
	const visit = (path) => {
		...
		for (const specifier of imports) {
			...
			const candidate = [
				...
				join(base, "index.ts"),
				join(base, "index.js"),
			].find((candidatePath) => existsSync(candidatePath) && statSync(candidatePath).isFile());
			if (!candidate) throw new Error(`extension import is missing: ${specifier} from ${path}`);
			visit(candidate);
		}
	};
	visit(entrypoint);
	return sha256CanonicalJson(
		[...visited].sort().map((path) => [relative(packageRoot, path), hashFile(path)]),
	);
}
...
function piProfile(request, parsed) {
	const agentDir = request.env.PI_CODING_AGENT_DIR
		...
		: request.env.HOME
			...
			: undefined;
	if (!agentDir)
		throw new Error("Pi agent directory is unavailable from the effective environment");
	for (const path of [
...
function locatePiHostPackage(executable) {
	const adjacent = resolve(dirname(executable), "../@earendil-works/pi-coding-agent");
	if (existsSync(join(adjacent, "package.json"))) return realpathSync(adjacent);
	...
	throw new Error("cannot resolve the host package from the Pi executable");
}
...
async function preflightPi(request) {
	...
	let profile;
	try {
		profile = piProfile(request, parsed);
	} catch (error) {
		return fail(failureCodes.has(error.code) ? error.code : "unverifiable-profile", error.message);
	}
	let hostRoot;
	try {
		hostRoot = locatePiHostPackage(request.executable);
	} catch (error) {
		return fail("unverifiable-profile", error.message);
	}
	const hostManifest = object(
		parseStrictJson(readFileSync(join(hostRoot, "package.json"), "utf8"), METADATA_MAX_BYTES),
		"Pi host manifest",
	);
	if (hostManifest.name !== "@earendil-works/pi-coding-agent" || hostManifest.version !== "0.84.4")
		return fail("unsupported-host", "resolved Pi host package identity is not the pinned package");
	const hostHash = hashDirectory(hostRoot);
	const executableHash = hashFile(request.executable);
	const ownerClosure = profile.extensions.find((entry) => entry.entrypoint === piOwner)[
		"closure-sha256"
	];
	const adapterHash = sha256CanonicalJson({
		ownerClosure,
		packageManifest: hashFile(join(packageRoot, "package.json")),
		packageLock: hashFile(join(packageRoot, "pnpm-lock.yaml")),
	});
	const hookFact = {
		"host-package": hostManifest.name,
		"host-package-version": hostManifest.version,
		"host-package-sha256": hostHash,
		extensions: profile.extensions,
		"prompt-owner-entrypoint": profile.owner,
...
try {
	...
	response =
		request.harness === "codex" ? await preflightCodex(request) : await preflightPi(request);
} catch (error) {
	response = fail("unverifiable-profile", error instanceof Error ? error.message : String(error));
}
