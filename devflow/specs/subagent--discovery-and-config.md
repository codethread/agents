...
- **SPEC-003.B1:** Discover agents from bundled package content plus optional user, project, and `--agents-dir` directories.
- **SPEC-003.B2:** Return a uniform `AgentConfig` shape regardless of source file location.
- **SPEC-003.B3:** Present Pi with one effective merged list of available delegation targets.
- **SPEC-003.B4:** Re-read agent markdown and swarm configuration on each discovery call so edits take effect without restarting Pi.
- **SPEC-003.B5:** Discover user/project/package swarm definitions that fan out one tool call to multiple named agent members.
- **SPEC-003.B6:** Reject ambiguous effective target catalogs where an agent and a swarm claim the same name.
- **SPEC-003.B7:** Allow agents to be hidden from the parent prompt inventory without removing them from discovery or explicit name-based execution.
- **SPEC-003.B8:** Preserve source metadata for debug output and project-agent confirmation.
- **SPEC-003.B9:** Allow direct top-level selection of one discovered agent config via `--agent <name>`.
- **SPEC-003.B10:** Normalize legacy Claude-flavored tool names while treating agent `tools` as an exact allowlist across built-in and extension tools.
- **SPEC-003.B11:** Parse declared model policy at the discovery boundary into ordered candidates and per-agent config errors.
- **SPEC-003.B12:** Parse Claude Code-style `mcpServers` frontmatter into normalized server configs, recording per-agent config errors for malformed declarations.
- **SPEC-003.B13:** Connect an adopted/spawned agent's MCP servers and expose their tools to that agent, namespaced to avoid collisions.
- **SPEC-003.B14:** Apply deterministic source precedence so agent and swarm name collisions resolve predictably, including repeatable `--agents-dir` roots.

### SPEC-003.P4 Non-Goals

- **SPEC-003.B15:** Executing agent tasks or managing agent process lifecycle. That belongs to the subagent runtime in `pi/extensions/tools/subagent/`.
- **SPEC-003.B16:** Validating rich agent schemas beyond the required frontmatter fields. Discovery assumes frontmatter is parseable and that `tools`, when present, behaves like a string.
- **SPEC-003.B17:** Merging multiple definitions of the same agent across sources. Agent-agent collisions are resolved by replacement, not composition.
- **SPEC-003.B18:** Executing swarm members, aggregating member output, or managing swarm resume. That belongs to the subagent runtime in `pi/extensions/tools/subagent/`.
- **SPEC-003.B19:** Supporting arbitrary external tool names. Tool normalization is intentionally narrow and conservative.
- **SPEC-003.B20:** Executing model-chain retry behavior; discovery only parses and carries model policy for runtime validation/execution.
- **SPEC-003.B21:** Exposing user/project/both discovery scopes to Pi-facing runtime selection.

## SPEC-003.P5 2. Design Decisions

- **SPEC-003.D1 Decision:** Agents are defined as markdown files with frontmatter and prompt body.
...
- **SPEC-003.D2 Decision:** Pi sees one merged target list rather than choosing among discovery scopes.
...
- **SPEC-003.D3 Decision:** Repeatable `--agents-dir <root>` contributes external roots after project discovery, and later flags override earlier flags for same-name targets.
...
- **SPEC-003.D4 Decision:** `hidden: true` removes an agent from the parent prompt inventory only; it does not remove the agent from discovery.
...
- **SPEC-003.D5 Decision:** Direct top-level agent mode reuses the same merged discovery result as delegated subagent runs.
...
- **SPEC-003.D6 Decision:** Agent and swarm discovery is evaluated at call time, not cached for the process lifetime.
...
- **SPEC-003.D7 Decision:** Top-level `--agent` inheritance is derived from shared runtime-setting helpers rather than ad hoc field reads in the extension entrypoint.
...
- **SPEC-003.D8 Decision:** Explicit CLI flags override inherited agent fields on a per-field basis.
...
- **SPEC-003.D9 Decision:** Source precedence for same-kind definitions is implemented with a name-keyed `Map`.
...
- **SPEC-003.D10 Decision:** Source-specific lists are still returned for debugging.
  - **Rationale:** Humans sometimes need to understand where definitions came from even though Pi should not care.

- **SPEC-003.D11 Decision:** Discovery is best-effort for filesystem and JSON access, but not for all frontmatter shape errors.
...
- **SPEC-003.D12 Decision:** Agent `tools` is an exact allowlist over the full active tool namespace, not just built-ins.
...
- **SPEC-003.D13 Decision:** Tool normalization is only lossy for unsupported Claude-only aliases.
...
- **SPEC-003.D14 Decision:** Discovery does not rewrite model aliases.
...
- **SPEC-003.D15 Decision:** Declared `model` policy is parsed at the discovery boundary and malformed policy is recorded per agent.
...
- **SPEC-003.D16 Decision:** `mcpServers` is parsed at the discovery boundary into normalized configs, and malformed declarations are recorded per agent as `mcpServersError` rather than aborting discovery.
...
- **SPEC-003.D20 Decision:** Swarm member names resolve against the effective agent catalog, including agents colocated with the swarm definition.
...
- **SPEC-003.D21 Decision:** Swarms are advertised to the parent as normal callable targets, but direct top-level `--agent <name>` remains single-agent only unless explicitly extended later.
...
### SPEC-003.P8 Source locations and precedence

Discovery supports four agent sources:

- **SPEC-003.B52:** **package** — bundled agents under repo-level `pi/agents/`.
- **SPEC-003.B53:** **user** — agents under `~/.pi/agent/agents`
- **SPEC-003.B54:** **project** — agents under the nearest ancestor `.pi/agents`
- **SPEC-003.B55:** **flag** — each `--agents-dir <root>` contributes `<root>/agents`

Swarm sources mirror this model:

- **SPEC-003.B56:** **package** — package-provided swarm folders, when shipped
- **SPEC-003.B57:** **user** — swarm folders under `~/.pi/agent/swarms/<name>/swarm.json`
- **SPEC-003.B58:** **project** — swarm folders under the nearest ancestor `.pi/swarms/<name>/swarm.json`
- **SPEC-003.B59:** **flag** — each `--agents-dir <root>` contributes `<root>/swarms/<name>/swarm.json`
...
- **SPEC-003.B22:** package agents/swarms are inserted first
- **SPEC-003.B23:** user agents/swarms overwrite package definitions of the same kind with the same name
- **SPEC-003.B24:** project agents/swarms overwrite both package and user definitions of the same kind with the same name
- **SPEC-003.B60:** `--agents-dir` roots are applied last in CLI order, so later flags overwrite earlier flags for the same kind and name
...
### SPEC-003.P9 Effective vs source-specific views

Discovery returns multiple views from one pass:

- **SPEC-003.B61:** `agents` — the effective merged list of single-agent targets
- **SPEC-003.B62:** `swarms` — the effective merged list of swarm targets
- **SPEC-003.B63:** `userAgents` — raw user-defined agents for debug inspection
- **SPEC-003.B64:** `projectAgents` — raw project-local agents for debug inspection
- **SPEC-003.B65:** `userSwarms` — raw user-defined swarms for debug inspection
- **SPEC-003.B66:** `projectSwarms` — raw project-local swarms for debug inspection
- **SPEC-003.B67:** `projectAgentsDir` — nearest project agent directory, if present
- **SPEC-003.B68:** `projectSwarmsDir` — nearest project swarm directory, if present
...
### SPEC-003.P10 Model policy parsing
...
- **SPEC-003.B69:** `$VAR`
- **SPEC-003.B70:** `!$VAR`
- **SPEC-003.B71:** `$VAR == "value"`
- **SPEC-003.B72:** `$VAR != 'value'`
...
### SPEC-003.P11 Runtime integration boundary

The subagent runtime uses discovery results in four places:

- **SPEC-003.B73:** `before_agent_start` discovers targets on demand and injects a terse XML list of visible subagent/swarm names and descriptions into the parent agent system prompt.
...
- **SPEC-003.B27:** a collision-free effective namespace of visible and hidden single-agent targets
- **SPEC-003.B28:** a collision-free effective namespace of swarm targets
- **SPEC-003.B29:** source-specific user/project views for debug rendering
- **SPEC-003.B30:** project directory metadata for debug rendering and project-source confirmation
- **SPEC-003.B31:** per-agent model-policy parse errors where malformed model frontmatter should be surfaced without discarding the whole catalog
...
## SPEC-003.P13 5. Interfaces

The `pi/extensions/tools/subagent/` module provides the API used by the subagent tool and direct `--agent` startup mode to:

- **SPEC-003.B32:** discover the effective target catalog for a working directory
- **SPEC-003.B33:** format visible targets for parent prompt injection and debug output
- **SPEC-003.B34:** resolve names to either a single agent or a swarm target
- **SPEC-003.B35:** derive runtime settings from a selected agent
- **SPEC-003.B36:** parse explicit CLI overrides for direct `--agent` inheritance
- **SPEC-003.B37:** format the selected agent prompt wrapper for top-level sessions

Behavioral contract:

- **SPEC-003.B38:** bundled package agents are always considered
- **SPEC-003.B39:** user agents are always considered
- **SPEC-003.B40:** project agents are loaded from the nearest ancestor `.pi/agents`, when present
- **SPEC-003.B41:** each `--agents-dir <root>` contributes optional `<root>/agents` and `<root>/swarms` directories after shell expansion and cwd-relative resolution
- **SPEC-003.B42:** user/project/package/flag swarms are loaded from source-specific `swarms/<name>/swarm.json` folders
- **SPEC-003.B43:** swarm-local markdown files are loaded before validating that swarm's member list
- **SPEC-003.B44:** project directory metadata reports the nearest project directories even when no project targets are ultimately loaded
- **SPEC-003.B45:** unreadable directories, unreadable files, and invalid JSON settings fail closed by omission rather than throwing
- **SPEC-003.B46:** agent frontmatter is expected to be parseable, and `tools` is expected to be string-like when present; parse/type errors in those paths are not isolated per file by the current implementation
- **SPEC-003.B47:** cross-kind name collisions and unknown swarm members are hard errors surfaced during discovery/session initialization

### SPEC-003.P14 Accepted agent frontmatter

Discovery relies on these frontmatter fields:

- **SPEC-003.B76:** `name` — required; files without it are skipped
- **SPEC-003.B77:** `description` — required; files without it are skipped
- **SPEC-003.B78:** `meta` — optional author-only string; ignored by discovery/runtime and not exposed to parent agents
- **SPEC-003.B79:** `hidden` — optional boolean/string flag; truthy `true` hides the agent from parent prompt inventory while keeping it discoverable and callable by explicit name
- **SPEC-003.B80:** `tools` — optional comma-separated string of tool names; built-ins and extension tools share the same namespace here, and omitted/blank resolves to an empty allowlist
- **SPEC-003.B81:** `model` — optional unified model policy: non-empty string, object with `id` and optional `when`, or non-empty ordered list of strings/objects. Discovery does not rewrite aliases.
- **SPEC-003.B82:** `mcpServers` — optional Claude Code-style list of single-key maps. Each entry maps a server name to either a remote config (`type` of `http`/`sse` with a `url` and optional `headers`) or a local stdio config (`command` with optional `args`/`env`). Malformed declarations are recorded as a per-agent `mcpServersError` and leave `mcpServers` empty without aborting discovery.

All remaining markdown body content is passed through as the agent system prompt.

### SPEC-003.P15 Runtime consumption
...
## SPEC-003.P16 7. Open Questions

- **SPEC-003.B48:** Should the frontmatter schema eventually support structured tool arrays instead of a comma-separated string?
- **SPEC-003.B49:** Should `formatAgentList` remain part of the public module surface if it is only a presentation helper?
- **SPEC-003.B50:** Should model alias resolution grow beyond OpenAI/OpenAI Codex-backed environments, or is conservative pass-through the intended long-term behavior?
- **SPEC-003.B51:** Should direct top-level `--agent <name>` ever support swarm targets, or should swarms remain delegation-only?

## SPEC-003.P17 8. Code Locations

- **SPEC-003.B83:** `pi/extensions/tools/subagent/` — discovery, formatting, direct-agent inheritance, and runtime consumption
- **SPEC-003.B84:** `pi/extensions/tools/subagent/mcp.ts` — `mcpServers` parsing, MCP client connection, namespacing, smoke test
- **SPEC-003.B85:** `pi/extensions/tools/subagent/mcp-runtime.ts` — Pi-side MCP tool registration for adopted/spawned agents
- **SPEC-003.B86:** `pi/agents/jira-mcp.md` — bundled example agent using the Atlassian MCP server
- **SPEC-003.B87:** `pi/agents/` — bundled package agents and swarms
