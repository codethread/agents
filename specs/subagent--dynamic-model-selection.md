...
- Keep one public config surface: `model`.
- Let `model` be omitted so the agent inherits the parent/default Pi model.
- Let `model` be a string, an ordered list of strings, an object, or an ordered list of objects.
- Support simple environment gates on model candidates using a safe expression subset.
- Fail loudly when declared model policy is malformed or produces no valid candidates.
- Validate declared candidates against Pi's normal model registry/resolution semantics.
- Let delegated subagent execution retry transient provider failures and advance through ordered candidates transparently to the calling agent.
- Preserve human/debug/session metadata about attempted candidates without exposing operational churn in the agent-visible response.
...
- **Decision:** The only public model-policy field is `model`.
...
- **Decision:** Remove custom model alias rewrites from the subagent config layer.
...
- **Decision:** Thinking level is candidate-local when included in a model suffix.
...
- **Decision:** Transient provider failures retry the same candidate up to three total attempts; deterministic availability failures advance immediately.
...
- **Decision:** Context-window overflow is terminal for the subagent call.
...
- **Decision:** Startup validation is strict, while runtime hot-reload validation is scoped to the requested target.
...
- **Decision:** Runtime swarm execution can return per-member config failures alongside successful members.
...
- **Decision:** Attempt details are operational metadata, not agent-visible content.
...
- attempt metadata: compact per-attempt records for session/debug/UI surfaces
...
- validates declared model policy during startup
- applies the first valid candidate when model inheritance is not suppressed by CLI override
...
- Should Pi expose structured provider error codes later so this extension can stop relying on text classifiers for runtime provider failures?
- Should direct top-level `--agent` eventually gain true generation-level retry/advance support if Pi adds a suitable lifecycle hook?
- Should the `when` grammar grow `&&`/`||` once real configs need it, or should users keep composing profiles into one env var?
