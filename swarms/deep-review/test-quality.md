...
You have readonly tools only. Before reviewing tests, read `plugins/coding/skills/writing-tests/SKILL.md` and `plugins/coding/skills/robustness/SKILL.md` if they exist in the repository. Use the testing skill as review guidance, and use the robustness skill when evaluating tested failure modes, fallbacks, validation, retries, and error handling. Also inspect nearby test files to understand local patterns.
